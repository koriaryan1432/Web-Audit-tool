...
export async function sendPlanUpgradedEmail(userId: string, newPlan: Plan): Promise<void> {
  try {
    ...
    if (!user) { console.warn(`[Email] User ${userId} not found`); return; }
    ...
    const featuresList = features.length > 0
      ? `<ul style="margin:0 0 16px;padding-left:0;list-style:none;">${features.map((f) => `<li style="font-size:14px;color:#1E293B;margin-bottom:8px;padding:8px 12px;background:#F0FDF4;border-radius:6px;">${f}</li>`).join('')}</ul>`
      : '';
    const content = `${heading(`You're now on ${newPlan}! 🎉`)}${paragraph(`Thanks ${firstName}! Your SiteGrade account has been upgraded to the <strong>${newPlan} plan</strong> (${price}).`)}${divider()}${heading(`What's unlocked on ${newPlan}`, 2)}${featuresList}${divider()}${ctaButton('Go to Dashboard →', `${appUrl}/dashboard`)}${divider()}<p style="margin:0;font-size:13px;color:#64748B;text-align:center;"><a href="${appUrl}/unsubscribe?token=${userId}" style="color:#64748B;">Unsubscribe from billing emails</a></p>`;
    await sendEmail({
      to: user.email,
      subject: `You're now on SiteGrade ${newPlan} — here's what's unlocked`,
      html: emailWrapper(content, `Your ${newPlan} plan is now active.`),
    });
