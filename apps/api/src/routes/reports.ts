...
reportsRouter.post('/audits/:id/pdf', authMiddleware, planGuard('PRO'), async (c) => {
  ...
  const pdfQueue = new Queue('pdf-queue', { connection: getRedisConnection() as any });
  const job = await pdfQueue.add(
    'generate-pdf',
    { auditId, userId: user.id },
    { jobId: `pdf-${auditId}`, attempts: 3, backoff: { type: 'exponential', delay: 5000 }, removeOnComplete: { age: 86400 }, removeOnFail: { age: 86400 * 7 } }
  );
  return c.json({ data: { jobId: job.id, status: 'generating', message: 'PDF generation queued. Check back in 30-60 seconds.' } }, 202);
});
...
reportsRouter.get('/audits/:id/pdf', authMiddleware, async (c) => {
  const user = c.get('user');
  const { id: auditId } = c.req.param();
  const audit = await prisma.audit.findUnique({ where: { id: auditId }, select: { userId: true } });
  if (!audit) throw new NotFoundError('Audit not found');
  if (audit.userId !== user.id) throw new ForbiddenError('Access denied');
  const report = await prisma.report.findUnique({ where: { auditId }, select: { pdfUrl: true, createdAt: true } });
  if (!report || !report.pdfUrl) return c.json({ data: { status: 'generating', pdfUrl: null } });
  return c.json({ data: { status: 'ready', pdfUrl: report.pdfUrl, generatedAt: report.createdAt.toISOString() } });
});
