/**
 * Billing service — Stripe <-> DB sync helpers.
 * All functions are no-ops / throw gracefully when Stripe is not configured.
 *
 * NOTE: The Prisma schema stores stripe_customer_id on the `subscriptions`
 * table, NOT on `users`. This service reflects that correctly.
 */
...
/** Map Stripe price IDs -> SiteGrade plan enum */
 ...
 * Looks up the existing stripe_customer_id from the subscriptions table.
 */
export async function getOrCreateStripeCustomer(userId: string): Promise<string> {
  const stripe = getStripe();

  // Check if a subscription record already has a customer ID
  ...
  // Fetch user details to create the Stripe customer
  ...
  const customer = await stripe.customers.create({
    ...
    metadata: { userId: user.id, source: 'sitegade' },
  });
...
/** Get user ID from Stripe customer ID (via subscriptions table) */
